...
export default function ConfirmRegistration() {
  const navigate = useNavigate();
  const { data, updateData, fullName: fullNameForeign } = useRegistration();
  const { submitInfo, confirmRegistration, submitting, confirming } = useRegistrationStore();
  ...
  const handleRegister = async () => {
    ...
    const infoSuccess = await submitInfo(signupData);
    if (!infoSuccess) return;

    // Step 4 — confirm & commit, ได้ accessToken กลับมา (เก็บใน localStorage ให้แล้วใน store)
    const confirmResult = await confirmRegistration(data.profileId);
    if (confirmResult) {
      // ไป Phase 2 ต่อ (ตั้งคำถามความปลอดภัย) — ปรับ path ให้ตรงกับ router จริงของโปรเจกต์
  ...
  return (
    <div className="min-h-dvh bg-gray-50">
      <PageContainer>
        ...
        <StepFooter
          singleButton
          singleLabel={isBusy ? (submitting ? 'Submitting...' : 'Confirming...') : 'Register'}
          onSingle={handleRegister}
          nextDisabled={!data.confirmTermsAccepted || isBusy}
        />
      </PageContainer>
    </div>
  );
