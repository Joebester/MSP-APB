...
export default function KycMepormStep() {
  const navigate = useNavigate();
  const { fullName, data } = useRegistration();
  const { verifyMepom, verifyingMepom, mepomError } = useRegistrationStore();
  const [usernameId, setUsernameId] = useState('');
  const [verified, setVerified] = useState(false);

  const handleVerify = async () => {
    const success = await verifyMepom(usernameId.trim());
    setVerified(success);
  };
    ...
    <div className="min-h-dvh bg-white">
      <PageContainer>
        ...
        <div className="flex flex-1 flex-col px-4 pb-6 sm:px-6">
          ...
          <div className="mx-auto mt-2 h-px w-full max-w-xs bg-gray-200" />
          <p className="mx-auto mt-4 max-w-sm text-center text-sm leading-relaxed text-gray-600">
            You have registered for an MSP Wallet account. Please verify your
            identity using your existing Meporm account.
          </p>
          ...
          <div className="mt-6 rounded-xl border border-gray-200 bg-gray-50 p-4">
            ...
            <div className="mt-3 space-y-4 border-t border-gray-200 pt-4">
              ...
              />
              <Input
                label="Meporm Username / ID"
                value={usernameId}
                onChange={(e) => setUsernameId(e.target.value)}
                placeholder="Enter your Meporm username"
                inputClassName="bg-white"
              />
            </div>
          </div>
          ...
          {!verified && (
            <div className="mt-6">
              <Button
                ...
                disabled={!usernameId.trim() || verifyingMepom}
              >
                {verifyingMepom ? 'Verifying...' : 'Verify with Meporm'}
              </Button>
            </div>
          )}
          ...
          {verified ? (
            <div className="mt-6 flex justify-center">
              <div className="inline-flex items-center gap-2 rounded-lg bg-green-50 px-4 py-2 text-sm font-medium text-green-600">
        ...
        <footer className="border-t border-gray-200 px-4 py-4 sm:px-6">
          <Button
            ...
            className="w-full text-teal-700"
            size="lg"
            onClick={() => (verified ? navigate('/security-questions') : navigate('/kyc'))}
            disabled={!verified}
          >
            {verified ? 'Continue' : 'Close'}
          </Button>
        </footer>
      </PageContainer>
    </div>
