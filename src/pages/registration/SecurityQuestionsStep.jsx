...
export default function SecurityQuestionsStep() {
  const navigate = useNavigate();
  const { data, updateData } = useRegistration();
  const { submitSecurityAnswers, submittingQuestions } = useRegistrationStore();
  ...
  const handleNext = async () => {
    const success = await submitSecurityAnswers(data.securityAnswers);
    if (success) {
      navigate('/terms');
    }
  };
    ...
    <div className="min-h-dvh bg-white">
      <PageContainer>
        ...
        <StepFooter
          onBack={() => navigate('/confirm-pin')}
          onNext={handleNext}
          nextDisabled={!canProceed || submittingQuestions}
        />
      </PageContainer>
    </div>
  );
